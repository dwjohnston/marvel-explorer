...
function parseCharacters(
  data: MarvelRequestResult<unknown>
): MarvelRequestResult<MarvelCharacter> {
  // Here we have the opportunity to check that the payload result is what we're actually expecting, before we coerce the type
...
export async function fetchCharacters(
  offset = 0
): Promise<MarvelRequestResult<MarvelCharacter>> {
  const res = await fetch(
    `${REACT_APP_MARVEL_API_PATH}/characters?apikey=${REACT_APP_PUBLIC_KEY}&ts=${REACT_APP_TS}&hash=${REACT_APP_HASH}&offset=${offset}`,
    {
      method: "GET",
      mode: "cors",
      headers: {
